...
pub fn render_oauth_page(options: &OAuthPageOptions) -> String {
    let title = escape_html(&options.title);
    let heading = escape_html(&options.heading);
    let message = escape_html(&options.message);
    let details = options.details.as_ref().map(|value| escape_html(value));
    let details_html = details
        .as_ref()
        .map(|value| format!(r#"<div class="details">{value}</div>"#))
        .unwrap_or_default();
...
pub fn oauth_success_html(message: &str) -> String {
    render_oauth_page(&OAuthPageOptions::new(
...
pub fn oauth_error_html(message: &str, details: Option<&str>) -> String {
    let mut options = OAuthPageOptions::new(
        "Prime Intellect authentication failed",
        "Authentication failed",
        message,
    );
    if let Some(details) = details {
        options = options.with_details(details);
    }
    render_oauth_page(&options)
}
...
mod tests {
    ...
    #[test]
    fn oauth_page_escapes_html_in_message_and_details() {
        let html = oauth_error_html("<failed> & \"bad\"", Some("code='oops'"));

        assert!(html.contains("&lt;failed&gt; &amp; &quot;bad&quot;"));
        assert!(html.contains("code=&#39;oops&#39;"));
        assert!(!html.contains("<failed>"));
    }

    #[test]
    fn oauth_success_html_matches_success_shell() {
        let html =
        ...
        assert!(html.contains("OpenAI authentication completed. You can close this window."));
        assert!(html.contains("aria-label=\"Prime Intellect\""));
    }

    #[test]
    fn oauth_error_html_omits_details_block_when_absent() {
