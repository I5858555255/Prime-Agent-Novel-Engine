...
- Added `plan_id` to `refine.run()` so a plan returned by `refine.preview()` is applied exactly as previewed instead of re-planned ([#684](https://github.com/PrimeIntellect-ai/prime-agent/issues/684)).
- Added `refine.preview()` to plan a continual-harness refinement and return the proposed edits without applying them ([#684](https://github.com/PrimeIntellect-ai/prime-agent/issues/684)).
