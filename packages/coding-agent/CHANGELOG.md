...
- Changed auto-retry to survive longer provider outages: 10 attempts by default (was 3) with full-jitter exponential backoff capped by the new `retry.maxBackoffMs` setting (default 60s), honoring the provider's `Retry-After` even beyond that cap and stopping early when it exceeds the new `retry.maxRetryAfterMs` (default 5 minutes).
