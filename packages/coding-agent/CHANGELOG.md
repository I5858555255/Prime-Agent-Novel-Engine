...
- Added a bundled `firecrawl` skill (web search and page extraction via the [Firecrawl](https://firecrawl.dev) API) that loads by default, giving the agent a way to read pages rather than only search them. Add a key via `/login` → **MCP Connections** → "Firecrawl (web search + scrape)"; it is stored with your other credentials and supplied to the kernel automatically. Exposes `firecrawl(query)` and `firecrawl.scrape(url)` for the open web; `firecrawl.developer(query)` to answer programming questions from GitHub issues, pull requests, READMEs and docs with the matched passages; and `firecrawl.research(query)`, `.inspect_paper(id)`, `.related_papers(id, intent)` and `.read_paper(id, question)` for literature work. Honors `FIRECRAWL_API_URL` for self-hosted instances, and can be disabled with `bundledSkills.firecrawl: false`.
