...
- Changed OpenAI API-key and Codex subscription sessions to let OpenAI compact the request input at the usual `contextWindow - reserveTokens` threshold instead of summarizing locally. One resolved threshold decides what the request carries, whether it replays an earlier checkpoint, and whether the local threshold still runs, so a session cannot stop a turn waiting for a compaction it never starts and cannot shorten its input with compaction turned off. A model whose context window is at or below `reserveTokens` sends no threshold and keeps local compaction; `openai/gpt-4` is the one built-in model in that state at the default 16384-token reserve. `/compact`, model-requested compaction, and context-overflow recovery still summarize locally ([#913](https://github.com/PrimeIntellect-ai/prime-agent/pull/913) by [@paulbrav](https://github.com/paulbrav)).
- Changed the context size shown in the status tray, `/context`, `/usage`, and the subagent rows to read "unknown" after an OpenAI server compaction, until a response measures the same input the next request will send. A response from a different model still counts when neither request replays a checkpoint, so switching models no longer drops the reported size to a whole-history estimate ([#913](https://github.com/PrimeIntellect-ai/prime-agent/pull/913) by [@paulbrav](https://github.com/paulbrav)).
- Changed local compaction to drop OpenAI server compaction checkpoints from the messages it retains. A retained checkpoint made the next request replay it and cut away everything before it, including the new compaction summary ([#913](https://github.com/PrimeIntellect-ai/prime-agent/pull/913) by [@paulbrav](https://github.com/paulbrav)).
- Added `compat.supportsServerCompaction` to model and provider config. It enables OpenAI server compaction on a custom `openai-responses` endpoint, or disables it on an official one, and it gates checkpoint capture and replay along with the request field ([#913](https://github.com/PrimeIntellect-ai/prime-agent/pull/913) by [@paulbrav](https://github.com/paulbrav)).
