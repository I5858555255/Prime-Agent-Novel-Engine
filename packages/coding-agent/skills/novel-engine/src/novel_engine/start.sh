...
# 支持从项目根目录或上级目录运行
if [ -d "$SCRIPT_DIR/小说工程" ]; then
    PROJECT_DIR="$SCRIPT_DIR/小说工程"
else
...
start_dashboard() {
    info "正在启动 Web 控制台..."
    cd "$PROJECT_DIR"
    python3 web_dashboard.py --host "$CDN_HOST" --port "$CDN_PORT" &
    DASHBOARD_PID=$!
...
run_test() {
    ...
    [ "$use_real" = "true" ] && info "使用真实 Agnes API" || info "使用 Mock LLM"

    cd "$PROJECT_DIR"
    python3 medium_test_runner.py $num_chapters $real_flag
}
...
run_production() {
    ...
    cd "$PROJECT_DIR"
    python3 production_runner.py $real_flag $num_chapters $start_from
}
...
show_status() {
    ...
    # 检查运行中的 Python 进程
    local prod_pid=$(pgrep -f "production_runner.py" 2>/dev/null || true)
    local med_pid=$(pgrep -f "medium_test_runner.py" 2>/dev/null || true)
    if [ -n "$prod_pid" ]; then
