...
describe("daemon worker supervisor monitoring", () => {
	...
	it("forwards a disabled telemetry policy even when an attach snapshot is cached", async () => {
		const activeSessionId = "active-telemetry";
		const summary = {
			id: activeSessionId,
			activeSessionId,
			lifecycle: "live",
		...
		const worker = {
			descriptor: { workerId: "worker-telemetry", lifecycle: "ready", pid: 1234 },
			summaries: new Map([[activeSessionId, summary]]),
			snapshotCache: new Map([[activeSessionId, result]]),
			transcriptCaches: new Map(),
			snapshotTransferFrames: new Map(),
		...
		const supervisor = Object.assign(Object.create(DaemonSupervisor.prototype), {
			...
			attachClient(
				...
				command: { type: "attach"; activeSessionId: string; telemetryPolicy: "disabled" },
			): Promise<unknown>;
		};

		await supervisor.attachClient(client, {
			type: "attach",
			activeSessionId,
