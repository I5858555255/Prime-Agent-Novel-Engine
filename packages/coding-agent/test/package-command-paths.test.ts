...
describe("package commands", () => {
	...
	let originalPrimeAgentPackageDir: string | undefined;
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `prime-agent-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		originalPrimeAgentPackageDir = process.env.PRIME_AGENT_PACKAGE_DIR;
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("prime-agent install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "prime-agent --help" or "prime-agent install <source> [-l]".');
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: prime-agent install <source> [-l]");
...
`,
		...
		process.env.PRIME_AGENT_PACKAGE_DIR = selfPackageDir;
...
`,
		...
		const activePackageName =
			PACKAGE_NAME === "@new-scope/prime-agent" ? "@newer-scope/prime-agent" : "@new-scope/prime-agent";
		vi.stubGlobal(
			"fetch",
...
`,
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain(`Updated prime-agent`);
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:prime-agent-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "prime-agent-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:prime-agent-formatter?");
			expect(stdout).not.toContain("Updated prime-agent-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:prime-agent-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
