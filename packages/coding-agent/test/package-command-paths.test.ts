...
import { ENV_AGENT_DIR, ENV_PACKAGE_DIR, PACKAGE_NAME, VERSION } from "../src/config.js";
...
`,
		...
		const activePackageName = PACKAGE_NAME === "prime-agent-next" ? "prime-agent-newer" : "prime-agent-next";
		vi.stubGlobal(
			"fetch",
...
`,
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:prime-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "prime-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:prime-formatter?");
			expect(stdout).not.toContain("Updated prime-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:prime-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
