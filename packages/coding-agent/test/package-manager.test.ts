...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.PRIME_AGENT_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.PRIME_AGENT_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .prime/agent skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .prime/agent", async () => {
...
Content`,
		...
		it("should resolve directory with package.json primeAgent.extensions in extensions setting", async () => {
			// Create a package with primeAgent.extensions in package.json
			...
			// Should find the extensions declared in package.json primeAgent.extensions
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should dedupe user skill entries when ~/.prime/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .prime/agent auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".prime\n");
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with primeAgent manifest", async () => {
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .prime/agent settings base", () => {
	...
	describe("pattern filtering in primeAgent manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json primeAgent.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		...
		it("should skip installing missing package sources when offline", async () => {
			process.env.PRIME_AGENT_OFFLINE = "1";
