/**
 * Tests for git-based package updates, specifically handling force-push scenarios.
 *
...
describe("DefaultPackageManager git update", () => {
	...
	let agentDir: string; // The agent directory where packages are installed
	let installedDir: string; // The installed package directory
	...
	const gitSource = "git:github.com/test/package";
	...
	beforeEach(() => {
		...
		installedDir = join(agentDir, "git", "github.com", "test", "package");
	...
	function setupRemoteAndInstall(sourceOverride?: string): void {
		...
		createCommit(remoteDir, "skill.md", "// v1", "Initial commit");
	...
	describe("normal updates (no force-push)", () => {
		it("should skip reset, clean, and install when already up to date", async () => {
			mkdirSync(remoteDir, { recursive: true });
			initGitRepo(remoteDir);
			writeFileSync(join(remoteDir, "package.json"), JSON.stringify({ name: "test-package", version: "1.0.0" }));
			createCommit(remoteDir, "skill.md", "// v1", "Initial commit");
		...
		it("should update to latest commit when remote has new commits", async () => {
			...
			expect(getFileContent(installedDir, "skill.md")).toBe("// v1");

			// Add a new commit to remote
			const newCommit = createCommit(remoteDir, "skill.md", "// v2", "Second commit");
			...
			expect(getFileContent(installedDir, "skill.md")).toBe("// v2");
		});
		...
		it("should handle multiple commits ahead", async () => {
			...
			createCommit(remoteDir, "skill.md", "// v2", "Second commit");
			createCommit(remoteDir, "skill.md", "// v3", "Third commit");
			const latestCommit = createCommit(remoteDir, "skill.md", "// v4", "Fourth commit");

			await packageManager.update();

			expect(getCurrentCommit(installedDir)).toBe(latestCommit);
			expect(getFileContent(installedDir, "skill.md")).toBe("// v4");
		});

		it("should update even when local checkout has no upstream", async () => {
			setupRemoteAndInstall();
			createCommit(remoteDir, "skill.md", "// v2", "Second commit");
			const latestCommit = createCommit(remoteDir, "skill.md", "// v3", "Third commit");
			...
			expect(getCurrentCommit(installedDir)).toBe(latestCommit);
			expect(getFileContent(installedDir, "skill.md")).toBe("// v3");
		});
	});
	...
	describe("force-push scenarios", () => {
		it("should recover when remote history is rewritten", async () => {
			...
			createCommit(remoteDir, "skill.md", "// v2", "Commit to keep");
			...
			const rewrittenCommit = createCommit(remoteDir, "skill.md", "// v2-rewritten", "Rewritten commit");

			// Update should succeed despite force-push
			await packageManager.update();

			expect(getCurrentCommit(installedDir)).toBe(rewrittenCommit);
			expect(getFileContent(installedDir, "skill.md")).toBe("// v2-rewritten");
		});
		...
		it("should recover when local commit no longer exists in remote", async () => {
			...
			createCommit(remoteDir, "skill.md", "// v2", "Commit A");
			createCommit(remoteDir, "skill.md", "// v3", "Commit B");
			...
			const newCommit = createCommit(remoteDir, "skill.md", "// v2-new", "New commit replacing A and B");

			// Update should succeed - the commits we had locally no longer exist
			await packageManager.update();

			expect(getCurrentCommit(installedDir)).toBe(newCommit);
			expect(getFileContent(installedDir, "skill.md")).toBe("// v2-new");
		});
		...
		it("should handle complete history rewrite", async () => {
			...
			createCommit(remoteDir, "skill.md", "// v2", "v2");
			createCommit(remoteDir, "skill.md", "// v3", "v3");
			...
			git(["reset", "--hard", "HEAD~2"], remoteDir);
			createCommit(remoteDir, "skill.md", "// rewrite-a", "Rewrite A");
			const finalCommit = createCommit(remoteDir, "skill.md", "// rewrite-b", "Rewrite B");

			// Should handle this gracefully
			await packageManager.update();

			expect(getCurrentCommit(installedDir)).toBe(finalCommit);
			expect(getFileContent(installedDir, "skill.md")).toBe("// rewrite-b");
		});
	});
	...
	describe("pinned sources", () => {
		it("should not update pinned git sources (with @ref)", async () => {
			...
			mkdirSync(remoteDir, { recursive: true });
			initGitRepo(remoteDir);
			const initialCommit = createCommit(remoteDir, "skill.md", "// v1", "Initial commit");
	...
	describe("scope-aware update", () => {
		it("should not install locally when source is only registered globally", async () => {
			...
			const projectGitDir = join(tempDir, ".prime", "agent", "git", "github.com", "test", "package");
