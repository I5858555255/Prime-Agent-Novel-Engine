...
describe("createAgentSession OpenRouter attribution headers", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `prime-agent-sdk-openrouter-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		cwd = join(tempDir, "project");
		agentDir = join(tempDir, "agent");
		mkdirSync(cwd, { recursive: true });
		mkdirSync(agentDir, { recursive: true });
		originalTelemetryEnv = process.env.PRIME_AGENT_TELEMETRY;
		delete process.env.PRIME_AGENT_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.PRIME_AGENT_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://github.com/PrimeIntellect-ai/prime-agent");
		expect(headers?.["X-OpenRouter-Title"]).toBe("Prime Agent");
