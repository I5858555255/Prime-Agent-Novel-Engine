...
| `PRIME_AGENT_TELEMETRY` | Override anonymous aggregate usage analytics with `1`/`true`/`yes` or `0`/`false`/`no` |
| `PRIME_AGENT_TELEMETRY_ENDPOINT` | Override the anonymous analytics ingestion endpoint |
| `DO_NOT_TRACK` | Disable anonymous usage analytics when set to `1`/`true`/`yes` |
