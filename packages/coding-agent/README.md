...
Prime Agent is a minimal terminal coding harness. Adapt it to your workflows, not the other way around, without having to fork and modify Prime Agent internals. Extend it with TypeScript [Extensions](#extensions), [Skills](#skills), [Prompt Templates](#prompt-templates), and [Themes](#themes). Put your extensions, skills, prompt templates, and themes in [Prime Agent Packages](#prime-agent-packages) and share them with others via npm or git.
...
You can also watch [this video](https://x.com/badlogicgames/status/2041151967695634619), where I show how I publish my `prime-agent` sessions.

I regularly publish my own `prime-agent` work sessions here:

- [badlogicgames/prime-agent on Hugging Face](https://huggingface.co/datasets/badlogicgames/prime-agent)
...
Prime Agent has two separate startup features:

- **Update check:** fetches GitHub release metadata to check whether a newer Prime Agent version exists. Disable it with `PRIME_AGENT_SKIP_VERSION_CHECK=1`.
- **Install/update telemetry:** disabled unless `PRIME_AGENT_INSTALL_TELEMETRY_URL` is set. When configured, opt out by setting `enableInstallTelemetry` to `false` in `settings.json`, or by setting `PRIME_AGENT_TELEMETRY=0`.
...
The default export can also be `async`. Prime Agent waits for async extension factories before startup continues, which is useful for one-time initialization such as fetching remote model lists before calling `agent.registerProvider()`.

**What's possible:**
- Custom tools (or replace built-in tools entirely)
- Subprime-agentss and plan mode
...
Prime Agent is aggressively extensible so it does not have to dictate your workflow. Features that other tools bake in can be built with [extensions](#extensions), [skills](#skills), or installed from third-party [Prime Agent packages](#prime-agent-packages). This keeps the core minimal while letting you shape Prime Agent to fit how you work.

**No MCP.** Build CLI tools with READMEs (see [Skills](#skills)), or build an extension that adds MCP support. [Why?](https://mariozechner.at/posts/2025-11-02-what-if-you-dont-need-mcp/)

**No subagents.** There are many ways to do this. Spawn Prime Agent instances via tmux, build your own with [extensions](#extensions), or install a package that does it your way.
...
prime-agent update [source|self]      # Update Prime Agent and packages (skips pinned packages)
prime-agent update --extensions       # Update packages only
prime-agent update --self             # Update Prime Agent only
prime-agent update --self --force     # Reinstall Prime Agent even if current
...
| `PRIME_AGENT_CACHE_RETENTION` | Set to `long` for extended prompt cache (Anthropic: 1h, OpenAI: 24h) |
