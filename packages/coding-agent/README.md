...
Then just talk to pi. By default, pi gives the model `ipython`. The model uses the persistent kernel to read files, run commands, edit code, inspect data, and call bundled Python skills such as `websearch` for keyless Exa MCP search. Add capabilities via [skills](#skills), [prompt templates](#prompt-templates), [extensions](#extensions), or [pi packages](#pi-packages).
