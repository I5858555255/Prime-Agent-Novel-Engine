 ...
 *
 * Usage:
 * - `prime-agent --preset plan` - start with plan preset
...
import type { ExtensionAPI, ExtensionContext } from "prime-agent";
import { DynamicBorder, getAgentDir } from "prime-agent";
...
export default function presetExtension(api: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.prime/agent/presets.json or .prime/agent/presets.json",
				"warning",
			);
			return;
		}
