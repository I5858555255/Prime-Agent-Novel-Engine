 ...
 * (logo + keybinding hints) with a custom component showing a custom logo.
 */

import type { ExtensionAPI, Theme } from "prime-agent";
import { VERSION } from "prime-agent";

// --- CUSTOM LOGO ---
function getPrimeAgentLogo(theme: Theme): string[] {
	// --- COLORS ---
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const logoLines = getPrimeAgentLogo(theme);
