...
export type TelemetryEventName = "agent started" | "agent run completed" | "agent session ended";
...
export function installAgentTelemetry(session: AgentSession, options: InstallAgentTelemetryOptions): void {
	...
	const finalizeRun = (): void => {
		...
		sink.capture("agent run completed", {
			...
			provider_category: lastAssistant ? providerCategory(lastAssistant.provider) : "unknown",
