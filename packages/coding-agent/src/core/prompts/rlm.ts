...
export function buildSubagentGuidance(): string {
	return [
		...
		"# For long-running fan-out, also persist a label/prompt/id registry under RLM_SESSION_DIR",
		"# so you can recover via agent_observe after kernel restart or compaction.",
		"# Example: Path(os.environ.get('RLM_SESSION_DIR', '.')).joinpath('subagents.jsonl').write_text(...)",
