...
export function buildRlmPrompt(options: RlmPromptOptions): string {
	...
	const availableModelSelectors = (options.availableModels ?? []).map((model) => `${model.provider}/${model.id}`);
	const parts = [
	...
	if (allowRecursion && hasIpython) {
		parts.push(
			...
			"A child inherits your current model by default. Only when the user or an applicable skill explicitly requests a different model, pass its exact provider-qualified selector, for example `rlm('sub-task', model='provider-id/model-id')`; do not choose a different model on your own.",
			availableModelSelectors.length > 0
				? `Authenticated model selectors available for subagents: ${availableModelSelectors.map((selector) => `\`${selector}\``).join(", ")}.`
				: "No authenticated model selectors are currently available for subagents.",
...
export function buildSubagentGuidance(): string {
	return [
		...
		"Subagents inherit the parent model. Use `model='provider-id/model-id'` only when the user or an applicable skill explicitly requests an authenticated model advertised above; the selected model remains attached to that child across later turns.",
