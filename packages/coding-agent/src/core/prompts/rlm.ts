...
export function buildSubagentGuidance(): string {
	return [
		...
		"Subagents are ephemeral by default: each `rlm(...)` call starts a fresh session. To reopen a subagent later with its saved chat history, make it persistent by passing a stable id: `await rlm('new instruction', persist=True, persistent_id='reviewer')`. The first call creates it; later calls with the same `persistent_id` reopen its saved session so it sees its system prompt + prior history + your new instruction. Set its role once with `system_prompt='...'`; that role is stored and re-applied on every reopen. Persistent subagents require a persisted (saved) session and live under the session's artifact dir.",
