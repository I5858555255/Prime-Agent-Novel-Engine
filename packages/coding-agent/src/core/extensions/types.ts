...
export interface ExtensionRuntimeState {
	...
	/**
	 * Extra env vars merged over process.env for pi.exec() subprocesses. Read at
	 * call time (not load time) so a host can scope per-session vars — e.g. the
	 * daemon supplying each session's herdr pane identity — without mutating the
	 * shared process.env. Returns undefined to use the parent env unchanged.
	 */
	getExecEnv?: () => Record<string, string> | undefined;
