 ...
 *
 * Spawns a Python IPython kernel as a child process and drives it via the
 * Jupyter ZMQ messaging protocol. State (variables, imports, loaded data)
 * persists across calls. This is the substrate Prime Agent's `ipython` tool
 * runs against.
 */
...
function pickPorts(n: number): number[] {
	// In production we'd bind to 0 and read back, but bind-to-0 + zeromq is
	// fiddly across all five sockets. Random high ports are sufficient given
	// kernel sessions are local and short-lived. If the picks collide we'll
	// surface that on socket connect.
...
export class KernelManager {
	...
	async start(): Promise<void> {
		...
		// ZMQ slow-joiner: give the kernel time to bind ports before we publish.
	...
	async execute(code: string, opts: ExecuteOptions = {}): Promise<ExecuteResult> {
		...
		// Serialize executions (Jupyter shell is request/reply per message).
	...
	async shutdown(): Promise<void> {
		...
		// Best-effort graceful shutdown via control channel; SIGTERM as fallback.
		try {
 ...
 *   2. ~/.prime-agent/kernel-venv/bin/python (the canonical install location;
 *      not yet auto-created — landing in a future PR)
 *   3. <repoRoot>/kernel-spike/.venv/bin/python (development fallback during
 *      M1 — set up via kernel-spike/setup.sh)
 */
export function resolveKernelPython(): string | null {
	...
	// Walk up from this file looking for kernel-spike/.venv/bin/python.
	// During development the file lives in src/ or dist/; either way the repo
	// root is some number of "../" hops away.
	for (let candidateDir = process.cwd(); candidateDir !== "/"; candidateDir = join(candidateDir, "..")) {
		const candidate = join(candidateDir, "kernel-spike", ".venv", "bin", "python");
