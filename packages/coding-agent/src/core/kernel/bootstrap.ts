...
const RUNTIME_READY_CHECK =
	"import rlm; assert hasattr(rlm, 'run'); assert callable(rlm); assert hasattr(rlm, 'rlm'); assert callable(rlm.rlm); assert callable(rlm.host_request); assert not hasattr(rlm, 'background'); assert not hasattr(rlm.rlm, 'background')";
...
async function ensureKernelPythonUncached(
	...
	if (override) {
		...
		if (!(await hasPrimeAgentRuntime(python)))
			missing.push("a current prime-agent-runtime with callable rlm.run and rlm.host_request");
		if (missing.length === 0) {
