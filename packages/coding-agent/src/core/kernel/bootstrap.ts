...
const RUNTIME_READY_CHECK = `import rlm; _harness_methods = ${JSON.stringify(REQUIRED_HARNESS_METHODS)}; assert hasattr(rlm, 'run'); assert callable(rlm); assert hasattr(rlm, 'rlm'); assert callable(rlm.rlm); assert hasattr(rlm, 'harness'); assert hasattr(rlm, 'get_harness_state'); assert hasattr(rlm.rlm, 'harness'); assert hasattr(rlm.rlm, 'get_harness_state'); assert all(callable(getattr(_harness, _method, None)) for _harness in (rlm.harness, rlm.rlm.harness) for _method in _harness_methods); assert not hasattr(rlm, 'background'); assert not hasattr(rlm.rlm, 'background')`;
...
async function ensureKernelPythonUncached(
	...
	if (override) {
		...
		if (!(await hasPrimeAgentRuntime(python))) {
			missing.push("a current prime-agent-runtime with callable rlm.run and explicit harness CRUD methods");
		}
		if (missing.length === 0) {
