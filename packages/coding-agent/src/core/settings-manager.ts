...
export interface RetrySettings {
	...
	baseDelayMs?: number; // default: 2000 (exponential backoff with full jitter)
	maxBackoffMs?: number; // default: 60000 (cap on the exponential backoff delay; 0 disables the cap)
	maxRetryAfterMs?: number; // default: 300000 (longest honored server Retry-After; longer stops retrying; 0 disables)
...
export class SettingsManager {
	...
	getRetrySettings(): {
		...
		return {
			...
			maxBackoffMs: this.settings.retry?.maxBackoffMs ?? 60000,
			maxRetryAfterMs: this.settings.retry?.maxRetryAfterMs ?? 300000,
		};
	}
