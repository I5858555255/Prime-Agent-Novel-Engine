...
By default, `install` and `remove` write to global settings (`~/.prime/agent/settings.json`). Use `-l` to write to project settings (`.prime/agent/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup.
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. Works for both global (`~/.prime/agent`) and project (`.prime/agent/`) scopes.
