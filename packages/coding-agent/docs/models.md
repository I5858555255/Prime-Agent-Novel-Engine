...
| `supportsServerCompaction` | Whether an `openai-responses` or `openai-codex-responses` endpoint takes OpenAI server-side compaction through `context_management`. An explicit value wins. Unset, it defaults to `true` for provider `openai` at `https://api.openai.com/v1` and provider `openai-codex` at `https://chatgpt.com/backend-api`, and `false` for every other endpoint, including a proxy in front of those. It also decides whether a compaction checkpoint is stored and whether a stored one is replayed, so `false` on a resumed session stops replay of checkpoints that session already holds and returns that model to local threshold compaction. See [Compaction](compaction.md). |
