...
| `supportsServerCompaction` | Whether an OpenAI Responses endpoint implements automatic server-side compaction through `context_management`. Every model on the official OpenAI and Codex endpoints enables this automatically. Custom endpoints default to `false`; an explicit value overrides endpoint detection. |
