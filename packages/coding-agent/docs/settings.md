...
Prime Agent reads this setting from disk for every provider request, so an external JSON edit applies to the next request without `/reload`. Chat Completions remains the default. When Responses is enabled, cancellation, context overflow, authentication, rate limiting, invalid input, and failures after streaming starts do not cross transports.
