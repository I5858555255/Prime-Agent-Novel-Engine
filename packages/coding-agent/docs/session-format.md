...
- **Version 3**: Renamed `hookMessage` role to `custom`
...
Session entries contain `AgentMessage` objects. Understanding these types is essential for parsing sessions and building integrations.
...
interface CustomMessage {
  role: "custom";
  customType: string;            // Custom entry identifier
  content: string | (TextContent | ImageContent)[];
  display: boolean;              // Show in TUI
  details?: any;                 // Custom metadata
  timestamp: number;
}
...
- `fromHook`: Legacy hook marker. `false`/`undefined` for pi-generated entries.
...
Custom state persistence. Does NOT participate in LLM context.

```json
{"type":"custom","id":"h8i9j0k1","parentId":"g7h8i9j0","timestamp":"2024-12-03T14:20:00.000Z","customType":"my-integration","data":{"count":42}}
```

Use `customType` to identify related custom entries on reload.

### CustomMessageEntry

Custom messages that DO participate in LLM context.

```json
{"type":"custom_message","id":"i9j0k1l2","parentId":"h8i9j0k1","timestamp":"2024-12-03T14:25:00.000Z","customType":"my-integration","content":"Injected context...","display":true}
```

Fields:
- `content`: String or `(TextContent | ImageContent)[]` (same as UserMessage)
- `display`: `true` = show in TUI with distinct styling, `false` = hidden
- `details`: Optional custom metadata (not sent to LLM)
...
Session metadata (e.g., user-defined display name). Set via the `/name` command.
...
- `appendCustomEntry(customType, data?)` - Custom state (not in context)
- `appendSessionInfo(name)` - Set session display name
- `appendCustomMessageEntry(customType, content, display, details?)` - Custom message (in context)
