...
Extensions are TypeScript modules that extend Prime Agent behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.
...
**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `api.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `api.registerCommand()`
...
| Package | Purpose |
|---------|---------|
| `prime-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
...
For distributed Prime Agent packages installed with `prime-agent install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (api: ExtensionAPI) {
  // Subscribe to events
  api.on("event_name", async (event, ctx) => {
...
If the factory returns a `Promise`, Prime Agent awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `api.registerProvider()` are flushed.
...
This pattern makes the fetched models available during normal startup and to `prime-agent --list-models`.
...
thinking level changes (settings, keybinding, api.setThinkingLevel())
...
```typescript
api.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `api.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
```typescript
import { isToolCallEventType } from "prime-agent";
...
```typescript
import { isBashToolResult } from "prime-agent";
...
```typescript
import { createLocalBashOperations } from "prime-agent";

api.on("user_bash", (event, ctx) => {
...
Request a graceful shutdown of api.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `prime-agent` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
- Captured old `prime-agent` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
`api.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `api.getAllTools()` and are callable by the LLM without `/reload`.

Use `api.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
// Simple text message
api.sendUserMessage("What is 2+2?");
...
api.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
api.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
```typescript
api.appendEntry("my-state", { count: 42 });
...
```typescript
api.setSessionName("Refactor auth module");
```
...
```typescript
// Set a label
api.setLabel(entryId, "checkpoint-before-refactor");
...
```typescript
const result = await api.exec("git", ["status"], { signal, timeout: 5000 });
...
api.setActiveTools(["bash", "edit"]); // Switch to optional shell/edit tools
```

`api.getAllTools()` returns `name`, `description`, `parameters`, and `sourceInfo`.
...
### api.getThinkingLevel() / api.setThinkingLevel(level)
...
```typescript
const current = api.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh"
...
If you need to discover models from a remote endpoint, prefer an async extension factory over deferring the fetch to `session_start`. Prime Agent waits for the factory before startup continues, so the registered models are available immediately, including to `prime-agent --list-models`.
...
```typescript
api.registerCommand("my-setup-teardown", {
...
Register tools the LLM can call via `api.registerTool()`. Tools are exposed through provider tool schemas and can have custom rendering.
...
Use `promptGuidelines` to add tool-specific bullets to the default system prompt. These bullets are included only while the tool is active (for example, after `api.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "prime-agent";
...
api.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via api.exec (captured from extension closure)
    const result = await api.exec("some-command", [], { signal });
...
```typescript
import { createBashTool, type BashOperations } from "prime-agent";
...
```typescript
import { createBashTool } from "prime-agent";
...
export default function (api: ExtensionAPI) {
  ...
  api.registerTool({ name: "db_connect", ... });
  api.registerTool({ name: "db_query", ... });
  api.registerTool({ name: "db_close", ... });
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/PrimeIntellect-ai/prime-agent/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "prime-agent";
...
- Prime Agent ids use the `app.*` namespace, for example `app.tools.expand`, `app.editor.external`, `app.session.rename`
...
```typescript
import { CustomEditor, type ExtensionAPI } from "prime-agent";
...
```typescript
import { highlightCode, getLanguageFromPath } from "prime-agent";
...
| `subagent/` | Spawn subagents | `registerTool`, `exec` |
| **Games** |||
...
| `event-bus.ts` | Inter-extension events | `api.events` |
| **Session Metadata** |||
