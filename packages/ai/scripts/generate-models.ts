...
async function generateModels() {
	...
	// Add missing Grok models until the upstream feed includes all typed test fixtures.
	const xaiFallbackModels: Model<"openai-completions">[] = [
		{
			id: "grok-3",
			name: "Grok 3",
			api: "openai-completions",
	...
	for (const model of xaiFallbackModels) {
		if (!allModels.some((m) => m.provider === "xai" && m.id === model.id)) {
