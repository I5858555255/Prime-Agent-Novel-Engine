...
**Ollama Cloud**: Uses the OpenAI-compatible API at `https://ollama.com/v1`. Set `OLLAMA_API_KEY`, then select a built-in Cloud model such as `gpt-oss:120b`. For a local Ollama server or a proxy, define a programmatic [custom model](#custom-models) with its own base URL.
