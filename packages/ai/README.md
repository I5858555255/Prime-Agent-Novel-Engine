...
| Prime Inference | `PRIME_API_KEY` (falls back to `~/.prime/config.json` from `prime login`) |
...
**Prime Inference**: Uses the OpenAI-compatible API at `https://api.pinference.ai/api/v1`. Set `PRIME_API_KEY`, pass an API key explicitly, or log in with the Prime CLI so `~/.prime/config.json` contains an `api_key`.
