...
const agentDirEnvName = "PRIME_AGENT_DIR";
const startupBenchmarkEnvName = "PRIME_AGENT_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific PRIME_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force PRIME_AGENT_OFFLINE=1 / PRIME_AGENT_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.PRIME_AGENT_OFFLINE = "1";
		env.PRIME_AGENT_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "prime-agent-startup-benchmark-")) : undefined;
