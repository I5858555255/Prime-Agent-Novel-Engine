 ...
 *   --analyze      Spawn Prime Agent subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("prime-agent", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn prime-agent: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".prime/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn Prime Agent subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".prime/agent/AGENTS.md");
